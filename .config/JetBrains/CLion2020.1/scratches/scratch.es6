(function () {
    ...
    var team1 = `Bolton U23`;
    var team2 = `Nottm Forest U23`;
    let k = 0;
    ...
    prts.some(p => {
        if (p.textContent === team1 || p.textContent === team2) k++;
        if (k === 2) return true
    });
    if (k != 2) return false;
    ...
    function meanHd(hd) {
        if (!hd.val) return 0.0;
        if (!hd.val2) return hd.val;
        return (hd.val + hd.val2) / 2;
    }
        ...
        if (odds.length === lbls.length) {
            for (let j = 0; j < odds.length; ++j) {
                ...
                if (has_rows && hdps.length && odds.length != hdps.length) {
                    ...
                    for (let tmp = unf_odds[j]; tmp; tmp = tmp.parentElement) {
                        ...
                        unf_hdps.some((unhdp, n) => {
                            for (let tmp2 = unhdp; tmp2; tmp2 = tmp2.parentElement) {
                ...
                if (special) {
                    ...
                    if (obj.hd) {
                        if (!obj.special_bettype || obj.special_bettype === SpecialBetType.SpecialBetType_UNKNOWN) {
                            if (obj.subcategory_name === 'Total') {
                                if (meanHd(hd) >= 0) obj.special_bettype = SpecialBetType.TOTAL_OVER;
                                else obj.special_bettype = SpecialBetType.TOTAL_UNDER
                            }
                            else if (obj.subcategory_name === 'Spread') {
                                if (meanHd(hd) >= 0) obj.special_bettype = SpecialBetType.HANDICAP_PLUS;
                    ...
                    log('not so special', hd);
                    obj.name = lbls[j];
                    if (hd) {
                        obj.hdp = hd
                        if (!obj.special_bettype || obj.special_bettype === SpecialBetType.SpecialBetType_UNKNOWN) {

                            if (obj.general_bettype === GeneralBetType.SPREAD) {
                                if (meanHd(hd) >= 0)
                                    obj.special_bettype = SpecialBetType.HANDICAP_PLUS;
                                else
                                    obj.special_bettype = SpecialBetType.HANDICAP_MINUS;
                            } else if (obj.general_bettype === GeneralBetType.TOTAL_POINTS || obj.general_bettype === GeneralBetType.TEAM_TOTAL_POINTS) {
                                if (meanHd(hd) >= 0)
                                    obj.special_bettype = SpecialBetType.TOTAL_OVER;
                                else
                                    obj.special_bettype = SpecialBetType.TOTAL_UNDER;
                            }
                            log ('SPECIAL BETTYPE SET', obj);
                        }

                    } else {
                        log ('set moneyline set');
                        if (!obj.special_bettype || obj.special_bettype === SpecialBetType.SpecialBetType_UNKNOWN)
                            obj.special_bettype = SpecialBetType.MONEYLINE_SPECIAL;
                    }
                }
    ...
    btns.forEach(function (item) {
        ...
        //Totals
            ...
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.MATCH_GOALS_2WAY_SOCCER)
        } else if (cat && (cat.textContent === 'Alternative Match Goals')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.ALTERNATIVE_MATCH_GOALS_2WAY_SOCCER)
        } else if (cat && (cat.textContent === 'First Half Goals')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.FIRST_HALF_GOALS_2WAY_SOCCER)
        } else if (cat && (cat.textContent.startsWith('Goal Line ('))) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.GOAL_LINE_2WAY_SOCCER)
        } else if (cat && (cat.textContent.startsWith('1st Half Goal Line ('))) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.FIRST_HALF_GOAL_LINE_2WAY_SOCCER)
        } else if (cat && (cat.textContent === 'Asian Corners')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.ASIAN_CORNERS_SOCCER)
        } else if (cat && (cat.textContent === '2-Way Corners')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.ALTERNATIVE_ASIAN_CORNERS_SOCCER)
        } else if (cat && (cat.textContent === '1st Half Asian Corners')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.FIRST_HALF_ASIAN_CORNERS_SOCCER)
        } else if (cat && (cat.textContent === '1st Half Corners')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.FIRST_HALF_CORNERS_3WAY_SOCCER)
        } else if (cat && (cat.textContent === 'Match Corners')) {
            output[cat.textContent] = parseme(item, SportType.SOCCER, GeneralBetType.TOTAL_POINTS, SpecialCategory_soccer.TOTAL_CORNERS_3WAY_SOCCER)
